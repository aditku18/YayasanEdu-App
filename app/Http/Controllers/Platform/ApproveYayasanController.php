...
class ApproveYayasanController extends Controller
{
    ...
    public function approve(Request $request, Foundation $foundation, TenantService $tenantService)
    {
        ...
        $user = $foundation->adminUser ?? User::where('email', strtolower($foundation->email))->first();
        if (!$user || !$user->hasVerifiedEmail()) {
            return redirect()->back()->with('warning', 'Yayasan tidak dapat di-approve karena email administrator (' . ($user->email ?? $foundation->email) . ') belum terverifikasi.');
        }

        // Pastikan dokumen sudah diverifikasi
        if (!$foundation->hasVerifiedDocuments()) {
            return redirect()->back()->with('warning', 'Yayasan tidak dapat di-approve karena dokumen belum diverifikasi. Silakan verifikasi dokumen terlebih dahulu.');
        }
