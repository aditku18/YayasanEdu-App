        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary-600 via-primary-500 to-indigo-500 p-8 md:p-12 text-white animate-fade-in-up shadow-2xl shadow-primary-500/20">
            ...
            <div class="relative z-10 text-center">
                <div class="flex items-center justify-center gap-3 mb-6">
                    ...
                    <h1 class="text-3xl md:text-4xl font-bold">Tagihan (Bills)</h1>
                </div>
                <p class="text-primary-100 text-lg leading-relaxed mb-8 max-w-2xl mx-auto">
                    Kelola tagihan vendor dan pembayaran pengeluaran yayasan dengan mudah dan transparan
                </p>
                
                <!-- Quick Stats -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center">
                        <p class="text-3xl font-black text-white">{{ $bills->count() }}</p>
                        <p class="text-primary-100 text-sm">Total Tagihan</p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center">
                        <p class="text-3xl font-black text-white">{{ $bills->where('status', 'pending')->count() }}</p>
                        <p class="text-primary-100 text-sm">Belum Bayar</p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center">
                        <p class="text-3xl font-black text-white">{{ $bills->where('status', 'paid')->count() }}</p>
            ...
            <div class="bg-gradient-to-r from-slate-50 to-white p-8 border-b border-slate-100">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-slate-900">Daftar Tagihan</h2>
                            <p class="text-slate-600">Kelola semua tagihan vendor dan pengeluaran</p>
                        </div>
                    </div>
                    <a href="{{ route('tenant.bill.create') }}" class="px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Judul Tagihan</th>
                            ...
                            @if($bills->count() > 0)
                                ...
                                <tr class="hover-lift animate-slide-in-left animate-delay-{{ loop->index % 3 + 1 }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-slate-900">{{ $bill->title }}</div>
                                        @if($bill->description)
                                            <div class="text-sm text-slate-500">{{ Str::limit($bill->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="text-sm text-slate-900">{{ $bill->vendor }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-slate-900">Rp {{ number_format($bill->amount, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-slate-900">{{ $bill->due_date->format('d M Y') }}</div>
                                        @if($bill->due_date < now() && $bill->status == 'pending')
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            ...
                                            @elseif($bill->status == 'overdue') status-overdue
                                            @else status-pending
                                            @endif">
                                            {{ $bill->status == 'paid' ? 'Lunas' : ($bill->status == 'overdue' ? 'Terlambat' : 'Menunggu') }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('tenant.bill.show', $bill) }}" class="text-blue-600 hover:text-blue-900 p-2 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                                </svg>
                                            </a>
                                            <a href="{{ route('tenant.bill.edit', $bill) }}" class="text-indigo-600 hover:text-indigo-900 p-2 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('tenant.bill.destroy', $bill) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900 p-2 bg-red-50 rounded-lg hover:bg-red-100 transition-colors" onclick="return confirm('Apakah Anda yakin ingin menghapus tagihan ini?')">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-slate-400 text-sm mt-1">Tambah tagihan pertama untuk memulai</p>
                                        </div>
                                    </td>
                                </tr>
                            @endif
