...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'EduSaaS') }} — {{ isset($foundation) ? $foundation->name : 'Tenant' }}</title>
    ...
    <body class="font-sans antialiased bg-gray-50" x-data="{ sidebarOpen: false }">
        <div class="min-h-screen flex">
            ...
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-72 bg-sidebar transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 flex flex-col">
                ...
                <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                    ...
                    <a href="{{ route('tenant.school.dashboard') }}" class="sidebar-link {{ request()->routeIs('tenant.school.dashboard') ? 'active' : '' }}">
                ...
                <div class="p-4 border-t border-white/10">
                    <div class="flex items-center gap-3 px-2 py-2">
                        <div class="w-8 h-8 bg-blue-500/20 rounded-full flex items-center justify-center">
                            <span class="text-sm font-medium text-blue-400">{{ substr(Auth::user()->name, 0, 1) }}</span>
                        </div>
                ...
                <header class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        @if(isset($trialDaysLeft))
                            <div class="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-sm font-medium">
        ...
        <style>
            ...
            .sidebar-link {
                @apply flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-slate-300 rounded-lg transition-colors hover:bg-white/5 hover:text-white;
            }
            .sidebar-link.active {
