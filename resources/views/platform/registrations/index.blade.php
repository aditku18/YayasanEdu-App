<x-platform-layout>
    <x-slot name="header">Registrasi Baru</x-slot>
    <x-slot name="subtitle">Kelola registrasi yayasan yang masuk</x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-100 text-sm font-medium">Total Registrasi</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['total_registrations']) }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['approved']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">Tingkat Approval</p>
                    <p class="text-3xl font-bold mt-1">{{ $stats['approval_rate'] }}%</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['trial']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $stats['growth_rate'] }}%</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        <form method="GET" action="{{ route('platform.registrations.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Cari Registrasi</label>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <option value="active" {{ request('status') == 'active' ? 'selected' : '' }}>Disetujui</option>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('platform.registrations.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-gray-900">Daftar Registrasi</h2>
                        <span class="text-sm text-gray-500">{{ $registrations->total() }} registrasi</span>
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-400">{{ $registration->subdomain }}.edusaaS.com</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        ...
                                        @elseif($registration->status == 'trial')
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('platform.registrations.show', $registration->id) }}" 
                                               class="text-indigo-600 hover:text-indigo-900">Detail</a>
                                            @if($registration->status == 'pending')
                                                <span class="text-gray-300">|</span>
                                                <form action="{{ route('platform.registrations.approve', $registration->id) }}" method="POST" class="inline">
                                                    @csrf
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p>Belum ada registrasi yang ditemukan</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="w-24 bg-gray-200 rounded-full h-2">
                                    <div class="bg-indigo-600 h-2 rounded-full" style="width: {{ $stats['total_registrations'] > 0 ? ($status['count'] / $stats['total_registrations']) * 100 : 0 }}%"></div>
                                </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Menunggu Persetujuan</h3>
                <div class="space-y-3">
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Registrasi Terbaru</h3>
                <div class="space-y-3">
                    @forelse($recentRegistrations as $recent)
                        <div class="flex items-center gap-3 pb-3 border-b border-gray-100 last:border-0">
                            <div class="w-2 h-2 bg-blue-500 rounded-full"></div>
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900">{{ $recent->name }}</p>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-gray-500">{{ $recent->email }}</span>
                    ...
                    @empty
                        <p class="text-sm text-gray-500">Belum ada registrasi</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="space-y-2">
                    <a href="{{ route('platform.registrations.index', request()->query()) . '&export=1' }}" 
                    ...
                    <form action="{{ route('platform.registrations.approve-all-pending') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui semua registrasi pending?')">
                        @csrf
                        <button type="submit" 
                    ...
                    <form action="{{ route('platform.registrations.send-reminder') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengirim reminder ke semua registrasi pending?')">
                        @csrf
                        <button type="submit" 
    ...
    <div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-xl bg-white">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-900">Tolak Registrasi</h3>
    ...
    <script>
        function showRejectModal(registrationId) {
            ...
            document.getElementById('rejectForm').action = `/platform/registrations/${registrationId}/reject`;
        }
