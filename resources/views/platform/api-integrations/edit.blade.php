<x-platform-layout>
    <x-slot name="header">Edit API Integration</x-slot>
    <x-slot name="subtitle">Update integration configuration and settings</x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form action="{{ route('platform.api-integrations.update', $integration) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $integration->name) }}" required
                            ...
                            <div>
                                ...
                                <select id="type" name="type" required
                                    ...
                                    @foreach($integrationTypes as $key => $label)
                                        <option value="{{ $key }}" {{ old('type', $integration->type) == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $integration->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="url" id="base_url" name="base_url" value="{{ old('base_url', $integration->base_url) }}" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                @error('base_url')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" id="api_key" name="api_key" value="{{ old('api_key', decrypt($integration->api_key)) }}" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                @error('api_key')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="url" id="webhook_url" name="webhook_url" value="{{ old('webhook_url', $integration->webhook_url) }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                @error('webhook_url')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $integration->is_active) ? 'checked' : '' }}
                ...
                <div class="mt-8 flex items-center justify-between pt-6 border-t border-gray-200">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('platform.api-integrations.show', $integration) }}" 
