<x-platform-layout>
    <x-slot name="header">Kelola Refund</x-slot>
    <x-slot name="subtitle">Pantau dan kelola permintaan refund</x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['total_refunds']) }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['approved_refunds']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['pending_refunds']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-100 text-sm font-medium">Penuh</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['full_refunds']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/>
        ...
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['rejected_refunds']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">Rp {{ number_format($stats['total_refund_amount'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-500">Refund Hari Ini</p>
                    <p class="text-2xl font-bold text-gray-900">Rp {{ number_format($stats['today_refund_amount'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        <form method="GET" action="{{ route('platform.refunds.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Cari Refund</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="ID refund atau alasan..." 
                ...
                <div>
                    ...
                    <select name="type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <option value="full" {{ request('type') == 'full' ? 'selected' : '' }}>Penuh</option>
                        <option value="partial" {{ request('type') == 'partial' ? 'selected' : '' }}>Sebagian</option>
                    </select>
                </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('platform.refunds.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-gray-900">Daftar Refund</h2>
                        <span class="text-sm text-gray-500">{{ $refunds->total() }} refund</span>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID Refund</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($refunds as $refund)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">#{{ str_pad($refund->id, 6, '0', STR_PAD_LEFT) }}</div>
                                            <div class="text-sm text-gray-500">{{ $refund->refund_id ?? 'Auto' }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $refund->transaction->foundation->name }}</div>
                                            <div class="text-sm text-gray-500">{{ $refund->transaction->foundation->email }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        @if($refund->type == 'full')
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('platform.refunds.show', $refund->id) }}" 
                                               class="text-indigo-600 hover:text-indigo-900">Detail</a>
                                            @if($refund->status == 'pending')
                                                <span class="text-gray-300">|</span>
                                                <button onclick="showApproveModal({{ $refund->id }})" 
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p>Belum ada refund yang ditemukan</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-orange-500 h-2 rounded-full" style="width: {{ $stats['total_refunds'] > 0 ? ($stats['full_refunds'] / $stats['total_refunds']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['full_refunds'] }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-purple-500 h-2 rounded-full" style="width: {{ $stats['total_refunds'] > 0 ? ($stats['partial_refunds'] / $stats['total_refunds']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['partial_refunds'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Recent Refunds -->
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Refund Terbaru</h3>
                <div class="space-y-3">
                    @php
                        $recentRefunds = $refunds->take(5);
                    @endphp
                    @forelse($recentRefunds as $refund)
                        <div class="flex items-center gap-3 pb-3 border-b border-gray-100 last:border-0">
                            <div class="w-2 h-2 {{ $refund->status == 'approved' ? 'bg-green-500' : ($refund->status == 'pending' ? 'bg-yellow-500' : 'bg-red-500') }} rounded-full"></div>
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900">#{{ str_pad($refund->id, 6, '0', STR_PAD_LEFT) }}</p>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-gray-500">Rp {{ number_format($refund->amount, 0, ',', '.') }}</span>
                                    <span class="text-xs text-gray-400">{{ $refund->created_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </div>
                    @empty
                        <p class="text-sm text-gray-500">Belum ada refund</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="space-y-2">
                    <a href="{{ route('platform.refunds.create') }}" class="w-full px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm block text-center">
                        <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
