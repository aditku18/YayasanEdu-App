<x-platform-layout>
    <x-slot name="header">Kelola Pembayaran</x-slot>
    <x-slot name="subtitle">Pantau dan kelola transaksi pembayaran</x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['total_payments']) }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['completed_payments']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-100 text-sm font-medium">Proses</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['processing_payments']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['failed_payments']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        <form method="GET" action="{{ route('platform.payments.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Cari Pembayaran</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Transaction ID, metode, atau nama yayasan..." 
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('platform.payments.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm text-gray-500">{{ $payments->total() }} pembayaran</span>
                    </div>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($payments as $payment)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $payment->foundation->name ?? 'N/A' }}</div>
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('platform.payments.show', $payment->id) }}" 
                                               class="text-indigo-600 hover:text-indigo-900">Detail</a>
                                            @if($payment->status == 'pending')
                                                <span class="text-gray-300">|</span>
                                                <button onclick="showConfirmModal({{ $payment->id }})" 
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p>Belum ada pembayaran yang ditemukan</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Berhasil</span>
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-green-500 h-2 rounded-full" style="width: {{ $stats['total_payments'] > 0 ? ($stats['completed_payments'] / $stats['total_payments']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['completed_payments'] }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-yellow-500 h-2 rounded-full" style="width: {{ $stats['total_payments'] > 0 ? ($stats['pending_payments'] / $stats['total_payments']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['pending_payments'] }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $stats['total_payments'] > 0 ? ($stats['processing_payments'] / $stats['total_payments']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['processing_payments'] }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                <div class="bg-red-500 h-2 rounded-full" style="width: {{ $stats['total_payments'] > 0 ? ($stats['failed_payments'] / $stats['total_payments']) * 100 : 0 }}%"></div>
                            </div>
                            <span class="text-sm font-medium text-gray-900">{{ $stats['failed_payments'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Recent Payments -->
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Pembayaran Terbaru</h3>
                <div class="space-y-3">
                    @php
                    ...
                    @forelse($recentPayments as $payment)
                        <div class="flex items-center gap-3 pb-3 border-b border-gray-100 last:border-0">
                            <div class="w-2 h-2 {{ $payment->status == 'success' ? 'bg-green-500' : ($payment->status == 'pending' ? 'bg-yellow-500' : 'bg-red-500') }} rounded-full"></div>
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900">{{ $payment->foundation->name ?? 'N/A' }}</p>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-gray-500">Rp {{ number_format($payment->amount, 0, ',', '.') }}</span>
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="space-y-2">
                    <a href="{{ route('platform.payments.create') }}" class="w-full px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm block text-center">
                        <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
