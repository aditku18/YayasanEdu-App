...
@section('title', 'Yayasan Suspended')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    {{-- Page Header --}}
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">Yayasan Suspended</h1>
        <p class="text-gray-600 mt-1">Kelola yayasan yang ditangguhkan, ditolak, atau kedaluwarsa</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-red-100 text-sm font-medium">Total Suspended</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['total_suspended']) }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-100 text-sm font-medium">Suspended</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['suspended']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-yellow-100 text-sm font-medium">Rejected</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['rejected']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">Expired</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['expired']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-gray-500 to-gray-600 rounded-xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-100 text-sm font-medium">Bulan Ini</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($stats['suspended_this_month']) }}</p>
                </div>
                <div class="bg-white/20 rounded-lg p-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-500">Sekolah Hilang</p>
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_schools_lost']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-500">Pengguna Hilang</p>
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_users_lost']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-500">Paket Premium Hilang</p>
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['had_premium_plans']) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        <form method="GET" action="{{ route('platform.foundations.suspended') }}" class="space-y-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('platform.foundations.suspended') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-gray-900">Daftar Yayasan Suspended</h2>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($foundations as $foundation)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        @if($foundation->status == 'suspended')
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ $foundation->updated_at->diffForHumans() }}
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p>Belum ada yayasan suspended yang ditemukan</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="w-24 bg-gray-200 rounded-full h-2">
                                    <div class="bg-indigo-600 h-2 rounded-full" style="width: {{ $stats['total_suspended'] > 0 ? ($status['count'] / $stats['total_suspended']) * 100 : 0 }}%"></div>
                                </div>
            ...
            <!-- Recent Suspensions -->
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 class="text-lg font-bold text-gray-900 mb-4">Suspensi Terbaru</h3>
                <div class="space-y-3">
                    @forelse($recentSuspensions as $suspension)
                        <div class="flex items-center gap-3 pb-3 border-b border-gray-100 last:border-0">
                            <div class="w-2 h-2 bg-red-500 rounded-full"></div>
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-900">{{ $suspension->name }}</p>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-gray-500">{{ $suspension->updated_at->diffForHumans() }}</span>
                    ...
                    @empty
                        <p class="text-sm text-gray-500">Belum ada suspensi</p>
                    @endforelse
                </div>
            </div>
