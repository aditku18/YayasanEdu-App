...
@section('title', 'Edit Gelombang PPDB')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Gelombang</h1>
            <p class="text-slate-500 mt-1">Perbarui data periode pendaftaran.</p>
        </div>
    </div>

    <form action="{{ $schoolSlug ? route('tenant.school.ppdb.waves.update', ['school' => $schoolSlug, 'wave' => $wave->id]) : route('tenant.ppdb.waves.update', $wave->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 premium-shadow space-y-6">
            <div class="space-y-1">
                ...
                <input type="text" name="name" value="{{ $wave->name }}" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    <label class="text-sm font-bold text-slate-700 ml-1">Tanggal Mulai</label>
                    <input type="date" name="start_date" value="{{ $wave->start_date }}" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">
                </div>
                <div class="space-y-1">
                    <label class="text-sm font-bold text-slate-700 ml-1">Tanggal Selesai</label>
                    <input type="date" name="end_date" value="{{ $wave->end_date }}" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    ...
                    <select name="academic_year_id" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium appearance-none">
                        ...
                        <option value="{{ $year->id }}" {{ $wave->academic_year_id == $year->id ? 'selected' : '' }}>{{ $year->name }} - {{ $year->semester }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-1">
                    ...
                    <select name="major_id" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium appearance-none">
                        ...
                        <option value="{{ $major->id }}" {{ $wave->major_id == $major->id ? 'selected' : '' }}>Khusus {{ $major->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    ...
                    <input type="number" name="registration_fee" value="{{ $wave->registration_fee }}" required class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="quota" min="1" value="{{ $wave->quota }}" placeholder="Misal: 40" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">
            ...
            <div class="space-y-1">
                ...
                <textarea name="description" rows="3" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-slate-900 focus:ring-2 focus:ring-primary-500 transition-all font-medium">{{ $wave->description }}</textarea>
            </div>
        </div>
